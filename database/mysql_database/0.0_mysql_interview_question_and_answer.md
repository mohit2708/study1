...
|       | --------------------------------------------------------------       |
|       | [What is storage engine in mysql?](#what-is-storage-engine-in-mysql) |
...
* A storage engine is a software component responsible for managing the storage, retrieval, and manipulation of data within a database table. 

#### **Types of mysql engine**
1. **InnoDB (Default)**
* Transactions: Supports full **ACID transactions** (BEGIN, COMMIT, ROLLBACK)
* Foreign Keys: Yes
* Row-level Locking: Only locks the rows involved in a query (not the whole table)
* Crash Recovery: Yes(Has automatic recovery using logs in case of a crash)
* Use Case: General-purpose engine for apps needing reliability and performance (e.g., e-commerce, banking)
