...
emma = Student('Emma')  # Output:- TypeError: __init__() missing 1 required positional argument: 'age'
...
* we can call the parent class constructor using super().__init__(). It ensures proper initialization of inherited attributes and is especially useful in multiple inheritance scenarios.
* Parent class constructor ko call karne ke liye super() use kiya jata hai.
```python
class Parent:
    def __init__(self, name):
        self.name = name
...
* **Note:-** HUM parent class ka naam use karke bhi constructor call kar sakte ho: Parent.__init__(self, name)
* Best Practice: super() use karna recommended hai, especially when working with inheritance and multiple inheritance.